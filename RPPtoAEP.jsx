...
// Licence : CC0
{
    ...
    var HeightSetting = 1080;
    var NewCompNameSetting = "NewComp(RPP2AEP)";
    var FpsSetting = 60;
    ...
    function run(){
        ...
        if(FpsSetting.text==""){FpsSetting.text=60;}else if(isNaN(FpsSetting.text)==true){FpsSetting.text=60;}       
        if(NewCompNameSetting.text!=""){var NewComp = app.project.items.addComp(NewCompNameSetting.text, Number(WidthSetting.text), Number(HeightSetting.text), 1, 120, Number(FpsSetting.text));}
        ...
        var ItemDuration = FootageAndCompObj[SelectedItemId].duration; //アイテムの長さ。画像や平面オブジェクトだと0になる。
        var FirstItemFlag = 1; // 各トラック最初のアイテムに実行するためのフラグ
        var j=1;
        var ExpFlag=0;
        var OddLinkFlag=0;
        for(var i=0;RPPItem.length>i;i++){
            ...
            if (FirstItemFlag==1&&i!=0){
                j++;
                if(NewCompNameSetting.text!=""){var NewComp = app.project.items.addComp(NewCompNameSetting.text+" "+String(j), Number(WidthSetting.text), Number(HeightSetting.text), 1, 120, Number(FpsSetting.text));}
            ...
            if(GenerateMode.selection.index==0){//RPPの長さ通り
                var addedlayer = NewComp.layers.add(FootageAndCompObj[SelectedItemId]);
            ...
            if(FirstItemFlag==1&&OddItemSetting.value==1){
                ...
                OddLinkFlag = 1; //ControlA直後のアイテムのフラグ。
            }
    ...
    function run2(){
        ...
        if(selectComp.length == 1&&selectComp[0] instanceof CompItem){
            if(selectComp[0].selectedProperties.length==0){alert("Select a Layer and Properties");return;}
        ...
        }else{alert("Select a Composition and Properties")}
    }
    function setting(){
        //GetFootageItem
        ...
        NewCompNameSetting = GUIItemGroup2.add("edittext",undefined,"")
        NewCompNameSetting.characters = 10;
        ...
        GUIItemGroup7.add("statictext",undefined,"height : ");
        HeightSetting = GUIItemGroup7.add("edittext",undefined,"1080");
        ...
        GenerateMode=GUIItemGroup6.add('dropdownlist', undefined, ["Generate as per RPP","Without Gaps","Don't Adjust Item Length"]);
        ...
        w.btn.onClick= function () {
            w.close();
            SelectedItemId = w.menu.selection.index;
            LoadRPP();
            // RPPファイルの読み込みRPPItem配列に時間を打ち込む
            if(GenerateMode2.selection.index==0){run();}
            else if(GenerateMode2.selection.index==1){run2();}
